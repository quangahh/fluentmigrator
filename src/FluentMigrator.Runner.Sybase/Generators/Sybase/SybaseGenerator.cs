...
namespace FluentMigrator.Runner.Generators.Sybase
{
    ...
    {
        public SybaseGenerator()
            : this(new SybaseQuoter())
        {
        }

        public SybaseGenerator(
            [NotNull] SybaseQuoter quoter)
        ...
        public SybaseGenerator(
            ...
            : this(new SybaseColumn(new SybaseTypeMap()), quoter, new EmptyDescriptionGenerator(), generatorOptions)
        {
        }

        protected SybaseGenerator(
        ...
        {
            var tableName = Quoter.QuoteTableName(expression.Index.TableName, expression.Index.SchemaName).Replace("[dbo].", string.Empty);
            return string.Format(DropIndex, tableName, Quoter.QuoteIndexName(expression.Index.Name));
        }

        public override string Generate(AlterSchemaExpression expression)
        {
            return CompatibilityMode.HandleCompatibilty("AlterSchema is not supported in Sybase");
        }
        ...
        {
            return CompatibilityMode.HandleCompatibilty("Sequences are not supported in Sybase");
        }
